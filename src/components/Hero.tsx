import {motion} from 'motion/react'
import { fadeUp, staggerContainer  } from '@/lib/animatio'
import { Button } from './ui/button'
import { SparkleIcon } from 'lucide-react'
export const Hero = () => {
    return (
        <motion.section initial='hidden' whileInView='visible' viewport={{ once: true, amount: 0.3 }} variants={staggerContainer(0)} className="pt-20" id='hero'>

            <motion.p variants={fadeUp} className='flex items-center justify-center py-1 gap-2'>
                <SparkleIcon size={15}/>
                <span>Introduction</span>
            </motion.p>
            <motion.h1 variants={fadeUp} className='text-4xl md: text-5xl lg:text-6xl font-semibold capitalize mt-2 max-w-3xl md:leading-16'>
                I'm <span className='text-primary'>Kene</span> Software Engineer.
            </motion.h1>

            <motion.div variants={fadeUp} className="mt-8">
                <Button asChild><a href="#project">My Projects</a></Button>

                <Button variant='outline'>
                      <a href="/public/Kene_s_Resume (3).pdf" download>Download Resume</a>
                    </Button>
            </motion.div>

            


        </motion.section>
    );
};